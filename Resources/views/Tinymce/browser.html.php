...
<head>
	...
	<?php foreach ($view['assetic']->stylesheets(
        ['@NyroDevUtilityBundle/Resources/public/css/nyrodevBrowser.css'],
        ['?yui_css'],
        ['output' => 'css/nyrodevBrowser.css']) as $url): ?>
...
<body>
	...
	<fieldset id="files" class="<?php echo $type; ?>">
        ...
        if ($dir) {
            $tmp = explode('/', $dir);
            $curDir = '';
            echo '<a href="'.$view['router']->generate($route, array_merge($routePrm, ['dir' => $curDir])).'">'.$view['translator']->trans('nyrodev.browser.rootDir').'</a>';
            foreach ($tmp as $d) {
                $curDir .= ($curDir ? '/' : '').$d;
                echo ' / <a href="'.$view['router']->generate($route, array_merge($routePrm, ['dir' => $curDir])).'">'.$d.'</a>';
            }
        }
        ?>
	...
	<?php foreach ($view['assetic']->javascripts(
        ['@NyroDevUtilityBundle/Resources/public/js/*jquery.js'],
        ['?yui_js'],
        ['output' => 'js/jquery.js']) as $url): ?>
