...
class UploadHandler
{
            ...
            $index, $content_range)
    {
        ...
        $uploaded_bytes = $this->fix_integer_overflow($content_range ? (int) $content_range[1] : null); // nyro update
            ...
            if (true !== $thumbResult) {
                ...
                if (!$this->options['ftp'] && !new_thumbnails_creation($targetPath, $targetFile, $_FILES['files']['name'][0], $this->options['config']['current_path'], $this->options['config'])) {
                    ...
                    if ($this->options['config']['image_resizing']) {
                        if (0 == $this->options['config']['image_resizing_width']) { // if width not set
                            if (0 == $this->options['config']['image_resizing_height']) {
                        ...
                        } elseif (0 == $this->options['config']['image_resizing_height']) { // if height not set
                    ...
                    $resize = false;
                    if (0 != $this->options['config']['image_max_width'] && $srcWidth > $this->options['config']['image_max_width'] && false === $this->options['config']['image_resizing_override']) {
                        $resize = true;
                        $srcWidth = $this->options['config']['image_max_width'];

                        if (0 == $this->options['config']['image_max_height']) {
                            $srcHeight = $this->options['config']['image_max_width'] * $srcHeight / $srcWidth;
                        }
                    }

                    if (0 != $this->options['config']['image_max_height'] && $srcHeight > $this->options['config']['image_max_height'] && false === $this->options['config']['image_resizing_override']) {
                        $resize = true;
                        $srcHeight = $this->options['config']['image_max_height'];

                        if (0 == $this->options['config']['image_max_width']) {
