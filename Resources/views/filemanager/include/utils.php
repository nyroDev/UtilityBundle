...
if (!isset($_SESSION['RF']) || 'RESPONSIVEfilemanager' != $_SESSION['RF']['verify']) {
    exit('forbiden');
}
...
if (!function_exists('trans')) {
    ...
    if (!isset($_SESSION['RF']['language'])
        || false === file_exists('lang/'.basename($_SESSION['RF']['language']).'.php')
...
function deleteDir($dir, $ftp = null, $config = null)
{
    if ($ftp) {
        ...
        foreach (scandir($dir) as $item) {
            ...
            // nyro update
            if (!deleteDir($dir.DIRECTORY_SEPARATOR.$item, $ftp, $config)) {
                return false;
            }
        }
    }
...
function create_img($imgfile, $imgthumb, $newwidth, $newheight = null, $option = 'crop', $config = [])
{
    ...
    if (file_exists($imgfile) || 0 === strpos($imgfile, 'http')) {
        if (0 === strpos($imgfile, 'http') || image_check_memory_usage($imgfile, $newwidth, $newheight)) {
...
function checkresultingsize($sizeAdded)
{
    ...
    if (false !== $config['MaxSizeTotal'] && is_int($config['MaxSizeTotal'])) {
...
function fix_filename($str, $config, $is_folder = false)
{
    ...
    if (!$config['empty_filename'] && 0 === strpos($str, '.') && false === $is_folder) {
        $str = 'file'.$str;
    }
...
function is_really_writable($dir)
{
    $dir = rtrim($dir, '/');
    // linux, safe off
    if (DIRECTORY_SEPARATOR == '/' && false == @ini_get('safe_mode')) {
