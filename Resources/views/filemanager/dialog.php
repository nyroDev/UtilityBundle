...
<div class="uploader">
    ...
    ?>
	    <ul class="nav nav-tabs">
			<li class="active"><a href="#tab1" data-toggle="tab"><?php echo trans('Upload_base');
    ?></a></li>
			<li><a href="#tab2" id="uploader-btn" data-toggle="tab"><?php echo trans('Upload_java');
    ?></a></li>
	    </ul>
    ...
    ?>"/>
		    ...
		    <div class="upload-help"><?php echo trans('Upload_base_help');
    ?></div>
			<?php if ($java_upload) {
    ?>
			</div>
			<div class="tab-pane" id="tab2">
		    	...
		    	<div class="upload-help"><?php echo trans('Upload_java_help');
    ?></div>
			<?php 
}
    ?>
...
<div class="navbar navbar-fixed-top">
    <div class="navbar-inner">
	    ...
	    <div class="brand"><?php echo trans('Toolbar');
    ?></div>
		...
		<div class="filters">
			...
			<div class="span4 half">
			    <?php if ($upload_files) {
    ?>
						    <button class="tip btn upload-btn" title="<?php echo  trans('Upload_file');
    ...
    ?>
				    <button class="tip btn paste-here-btn" title="<?php echo trans('Paste_Here');
    ?>"><i class="rficon-clipboard-apply"></i></button>
				    <button class="tip btn clear-clipboard-btn" title="<?php echo trans('Clear_Clipboard');
    ?>"><i class="rficon-clipboard-clear"></i></button>
				<?php 
}
    ?>
			</div>
    ...
    ?>" id="view0" data-value="0" title="<?php echo trans('View_boxes');
    ...
    ?>" id="view1" data-value="1" title="<?php echo trans('View_list');
    ...
    ?>" id="view2" data-value="2" title="<?php echo trans('View_columns_list');
    ...
    ?>
			    <input accesskey="f" type="text" class="filter-input <?php echo ($_GET['type'] != 1 && $_GET['type'] != 3) ? '' : 'filter-input-notype';
    ?>" id="filter-input" name="filter" placeholder="<?php echo fix_strtolower(trans('Text_filter'));
...
<!-- header div end -->
    ...
    if (!empty($bc)) {
        foreach ($bc as $k => $b) {
            ...
            if ($k == count($bc) - 2) {
                ...
                ?>
		<li><a href="<?php echo $link.$tmp_path?>"><?php echo $b?></a></li><li><span class="divider"><?php echo '/';
                ?></span></li>
	<?php 
            }
        }
    ...
    ?>" href="javascript:void('')" data-sort="name"><?php echo trans('Filename');
    ?></a></li>
		<li><a class="sorter sort-date <?php if ($sort_by == 'date') {
    echo ($descending) ? 'descending' : 'ascending';
}
    ?>" href="javascript:void('')" data-sort="date"><?php echo trans('Date');
    ?></a></li>
		<li><a class="sorter sort-size <?php if ($sort_by == 'size') {
    echo ($descending) ? 'descending' : 'ascending';
}
    ?>" href="javascript:void('')" data-sort="size"><?php echo trans('Size');
    ?></a></li>
		<li><a class="sorter sort-extension <?php if ($sort_by == 'extension') {
    echo ($descending) ? 'descending' : 'ascending';
}
    ?>" href="javascript:void('')" data-sort="extension"><?php echo trans('Type');
    ?></a></li>
		</ul>
	      </div>
	</li>
    ...
    ?>
	    <h4 id="help"><?php echo trans('Swipe_help');
    ?></h4>
	    <?php if (isset($folder_message)) {
    ?>
		<div class="alert alert-block"><?php echo $folder_message;
    ?></div>
	    <?php 
}
    ?>
    ...
    ?>" href="javascript:void('')" data-sort="size"><?php echo trans('Size');
    ?></a></div>
		<div class='img-dimension'><?php echo trans('Dimension');
    ?></div>
    ...
    ?>" href="javascript:void('')" data-sort="extension"><?php echo trans('Type');
    ?></a></div>
		<div class='file-operations'><?php echo trans('Operations');
    ?></div>
	    </div>
	    <?php 
}
        ...
        ?>
				  <a class="folder-link" href="dialog.php?<?php echo $get_params.rawurlencode($src).'&'.uniqid() ?>">
					  <div class="img-precontainer">
							...
							<img class="directory-img"  src="img/<?php echo $icon_theme;
        ?>/folder<?php if ($file == '..') {
    echo '_back';
}
        ?>.png" />
    ...
    ?>"><a class="folder-link" data-file="<?php echo $file ?>" href="dialog.php?<?php echo $get_params.rawurlencode($src).'&'.uniqid() ?>"><?php echo $file;
    ?></a></h4>
				    </div>
    ...
    ?>
				    <div class='file-extension'><?php echo trans('Type_dir');
    ?></div>
				    <figcaption>
					    <a href="javascript:void('')" class="tip-left edit-button rename-file-paths <?php if ($rename_folders && !$file_prevent_rename) {
    echo 'rename-folder';
}
    ?>" title="<?php echo trans('Rename')?>" data-path="<?php echo $rfm_subfolder.$subdir.$file;
    ...
    foreach ($files as $nu => $file_array) {
        ...
        $mini_src = str_replace($thumbs_base_path, $thumbs_base_path_url, $src_thumb);
    ...
    echo 'original-thumb';
            ...
            ?>">
				    <div class="filetype <?php echo $extension_lower ?> <?php if (in_array($extension_lower, $editable_text_file_exts)) {
    ...
    echo $img_width.'x'.$img_height;
            ...
            ?></div>
				<div class='file-extension'><?php echo $extension_lower;
            ?></div>
				<figcaption>
            ...
            ?>">
				    ...
				    <a title="<?php echo trans('Download')?>" class="tip-right" href="javascript:void('')" onclick="$('#form<?php echo $nu;
            ?>').submit();"><i class="icon-download"></i></a>
				    <?php if ($is_img && $src_thumb != '' && $extension_lower != 'tiff' && $extension_lower != 'tif') {
    ?>
				    <a class="tip-right preview" title="<?php echo trans('Preview')?>" data-url="<?php echo $src;
    ...
    ?>"
					title="<?php echo trans('Preview')?>" data-url="ajax_calls.php?action=media_preview&title=<?php echo $filename;
    ?>&file=<?php echo $current_path.$rfm_subfolder.$subdir.$file;
    ?>"
					href="javascript:void('');" ><i class=" icon-eye-open"></i></a>
						<?php 
} elseif ($preview_text_files && in_array($extension_lower, $previewable_text_file_exts)) {
    ?>
					    <a class="tip-right file-preview-btn" title="<?php echo trans('Preview')?>" data-url="ajax_calls.php?action=get_file&sub_action=preview&preview_mode=text&title=<?php echo $filename;
    ?>&file=<?php echo $current_path.$rfm_subfolder.$subdir.$file;
    ?>"
						href="javascript:void('');" ><i class=" icon-eye-open"></i></a>
						<?php 
} elseif ($googledoc_enabled && in_array($extension_lower, $googledoc_file_exts)) {
    ?>
					    <a class="tip-right file-preview-btn" title="<?php echo trans('Preview')?>" data-url="ajax_calls.php?action=get_file&sub_action=preview&preview_mode=google&title=<?php echo $filename;
    ?>&file=<?php echo $current_path.$rfm_subfolder.$subdir.$file;
    ?>"
						href="docs.google.com;" ><i class=" icon-eye-open"></i></a>

						<?php 
} elseif ($viewerjs_enabled && in_array($extension_lower, $viewerjs_file_exts)) {
    ?>
					    <a class="tip-right file-preview-btn" title="<?php echo trans('Preview')?>" data-url="ajax_calls.php?action=get_file&sub_action=preview&preview_mode=viewerjs&title=<?php echo $filename;
    ?>&file=<?php echo $current_path.$rfm_subfolder.$subdir.$file;
    ?>"
						href="docs.google.com;" ><i class=" icon-eye-open"></i></a>
...
<script>
    ...
    foreach ($files_prevent_duplicate as $key => $value): ?>
        files_prevent_duplicate[<?php echo $key;
    ?>] = '<?php echo $value;
    ?>';
    <?php endforeach;
    ?>
