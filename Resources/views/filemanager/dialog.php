...
if ('' == $subdir) {
    if (!empty($_COOKIE['last_position']) && false === strpos($_COOKIE['last_position'], '.')) {
...
if (!empty($_SESSION['RF']['subfolder'])
    && 0 !== strpos($_SESSION['RF']['subfolder'], '/')
    && false === strpos($_SESSION['RF']['subfolder'], '.')
) {
...
if (isset($_GET['descending'])) {
    $descending = $_SESSION['RF']['descending'] = 1 == fix_get_params($_GET['descending']);
} else {
...
$return_relative_url = isset($_GET['relative_url']) && '1' == $_GET['relative_url'] ? true : false;
...
if (isset($_GET['editor'])) {
    ...
    $editor = 0 == $_GET['type'] ? null : 'tinymce';
}
...
<body>
    ...
    <input type="hidden" id="return_relative_url" value="<?php echo true == $return_relative_url ? 1 : 0; ?>"/>
    ...
    <input type="hidden" id="clipboard" value="<?php echo (isset($_SESSION['RF']['clipboard']['path']) && null != trim($_SESSION['RF']['clipboard']['path'])) ? 1 : 0; ?>" />
    ...
    <div class="navbar-inner">
        ...
        <div class="filters">
            ...
            <div class="span4 half">
                ...
                <?php if ('apply_none' != $apply_type && $config['multiple_selection_action_button']) { ?>
            ...
            <div class="span2 half view-controller">
                <button class="btn tip<?php if (0 == $view) {
    echo ' btn-inverse';
} ?>" id="view0" data-value="0" title="<?php echo trans('View_boxes'); ?>"><i class="icon-th <?php if (0 == $view) {
    echo 'icon-white';
} ?>"></i></button>
                <button class="btn tip<?php if (1 == $view) {
    echo ' btn-inverse';
} ?>" id="view1" data-value="1" title="<?php echo trans('View_list'); ?>"><i class="icon-align-justify <?php if (1 == $view) {
    echo 'icon-white';
} ?>"></i></button>
                <button class="btn tip<?php if (2 == $view) {
    echo ' btn-inverse';
} ?>" id="view2" data-value="2" title="<?php echo trans('View_columns_list'); ?>"><i class="icon-fire <?php if (2 == $view) {
    echo 'icon-white';
} ?>"></i></button>
            </div>
            <div class="span6 entire types">
                <span><?php echo trans('Filters'); ?>:</span>
                <?php if (1 != $_GET['type'] && 3 != $_GET['type'] && $config['show_filter_buttons']) { ?>
                ...
                <label id="ff-item-type-5" title="<?php echo trans('Music'); ?>" for="select-type-5" class="tip btn ff-label-type-5"><i class="icon-music"></i></label>
                    <?php } ?>
                <?php } ?>
                <input accesskey="f" type="text" class="filter-input <?php echo (1 != $_GET['type'] && 3 != $_GET['type']) ? '' : 'filter-input-notype'; ?>" id="filter-input" name="filter" placeholder="<?php echo fix_strtolower(trans('Text_filter')); ?>..." value="<?php echo $filter; ?>"/><?php if ($n_files > $config['file_number_limit_js']) { ?><label id="filter" class="btn"><i class="icon-play"></i></label><?php } ?>

                <input id="select-type-all" name="radio-sort" type="radio" data-item="ff-item-type-all" class="hide"  />
                <label id="ff-item-type-all" title="<?php echo trans('All'); ?>" <?php if (1 == $_GET['type'] || 3 == $_GET['type']) { ?>style="visibility: hidden;" <?php } ?> data-item="ff-item-type-all" for="select-type-all" style="margin-rigth:0px;" class="tip btn btn-inverse ff-label-type-all"><?php echo trans('All'); ?></label>
...
<!-- header div end -->
    ...
    */ ?>
	...
	<li><small class="hidden-phone"><span title="<?php echo trans('total size').$config['MaxSizeTotal']; ?>"><?php echo trans('total size').': '.makeSize($sizeCurrentFolder).((false !== $config['MaxSizeTotal'] && is_int($config['MaxSizeTotal'])) ? '/'.$config['MaxSizeTotal'].' '.trans('MB') : ''); ?></span></small>
	</li>
	<?php } ?>
	</ul>
	</div>
	...
	<div class="span12">
		<?php if (($ftp && !$ftp->isDir($config['ftp_base_folder'].$config['upload_dir'].$rfm_subfolder.$subdir)) || (!$ftp && false === @opendir($config['current_path'].$rfm_subfolder.$subdir))) { ?>
		<br/>
		...
		<div class="file-name"><a class="sorter sort-name <?php if ('name' == $sort_by) {
        echo ($descending) ? 'descending' : 'ascending';
    } ?>" href="javascript:void('')" data-sort="name"><?php echo trans('Filename'); ?></a></div>
		<div class="file-date"><a class="sorter sort-date <?php if ('date' == $sort_by) {
        echo ($descending) ? 'descending' : 'ascending';
    } ?>" href="javascript:void('')" data-sort="date"><?php echo trans('Date'); ?></a></div>
		<div class="file-size"><a class="sorter sort-size <?php if ('size' == $sort_by) {
        echo ($descending) ? 'descending' : 'ascending';
    } ?>" href="javascript:void('')" data-sort="size"><?php echo trans('Size'); ?></a></div>
		<div class='img-dimension'><?php echo trans('Dimension'); ?></div>
		<div class='file-extension'><a class="sorter sort-extension <?php if ('extension' == $sort_by) {
        echo ($descending) ? 'descending' : 'ascending';
        ...
        foreach ($files as $file_array) {
            ...
            if ('..' == $file && '' != trim($subdir)) {
                ...
                echo 'dir';
            } ?> <?php if (!$config['multiple_selection']) { ?>no-selector<?php } ?>" <?php if (('' != $filter && false === stripos($file, $filter))) {
            ...
            foreach ($files as $nu => $file_array) {
                ...
                if ('.' == $file || '..' == $file || $file_array['extension'] == fix_strtolower(trans('Type_dir')) || !check_extension($file_array['extension'], $config) || ('' != $filter && $n_files > $config['file_number_limit_js'] && false === stripos($file, $filter))) {
                    continue;
                }
                ...
                if (0 === strlen($file_array['extension'])) {
                    $filename = $file;
                }
                if (!$ftp) {
                    $file_path = $config['current_path'].$rfm_subfolder.$subdir.$file;
                ...
                // nyro update
                $src = str_replace(
					array($config['thumbs_base_path'], $config['current_path']),
					array($config['thumbs_base_path_url'], $config['current_path_url']),
					$src);
                $src_thumb = str_replace(
					array($config['thumbs_base_path'], $config['current_path']),
					array($config['thumbs_base_path_url'], $config['current_path_url']),
					$src_thumb);
                $mini_src = str_replace($config['thumbs_base_path'], $config['thumbs_base_path_url'], $mini_src);
                ...
                if ((!(1 == $_GET['type'] && !$is_img) && !((3 == $_GET['type'] && !$is_video) && (3 == $_GET['type'] && !$is_audio))) && $class_ext > 0) {
                    ?>
            <li class="ff-item-type-<?php echo $class_ext; ?> file <?php if (!$config['multiple_selection']) { ?>no-selector<?php } ?>"  data-name="<?php echo $file; ?>" <?php if (('' != $filter && false === stripos($file, $filter))) {
                        echo ' style="display:none;"';
                    } ?>><?php
            $file_prevent_rename = false;
                    $file_prevent_delete = false;
