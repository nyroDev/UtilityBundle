...
if ('RESPONSIVEfilemanager' != $_SESSION['RF']['verify']) {
...
if (isset($_GET['action'])) {
    switch ($_GET['action']) {
        ...
        case 'copy_cut':
            ...
            $msg_sub_action = ('copy' == $_POST['sub_action'] ? trans('Copy') : trans('Cut'));
            $path = $config['current_path'].$_POST['path'];

            if (is_dir($path)) {
                // can't copy/cut dirs
                if (false === $config['copy_cut_dirs']) {
                ...
                // size over limit
                if (false !== $config['copy_cut_max_size'] && is_int($config['copy_cut_max_size'])) {
                ...
                if (false !== $config['copy_cut_max_count'] && is_int($config['copy_cut_max_count'])) {
                ...
                if (false === $config['copy_cut_files']) {
        ...
        case 'chmod':
            if ($ftp) {
                $path = $config['ftp_base_url'].$config['upload_dir'].$_POST['path'];
                if (
                    (1 == $_POST['folder'] && false === $config['chmod_dirs'])
                    || (0 == $_POST['folder'] && false === $config['chmod_files'])
                    || (false === is_function_callable('chmod'))) {
                ...
                $path = $config['current_path'].$_POST['path'];
                if (
                    (is_dir($path) && false === $config['chmod_dirs'])
                    || (is_file($path) && false === $config['chmod_files'])
                    || (false === is_function_callable('chmod'))) {
                    response(sprintf(trans('File_Permission_Not_Allowed'), (is_dir($path) ? trans('Folders') : trans('Files')), 403).AddErrorLocation())->send();
                    exit;
                }
            ...
            <form id="chmod_form">
                <table class="table file-perms-table">
                    ...
                    <tbody>
                        <tr>
                            <td>'.trans('User').'</td>
                            <td><input id="u_4" type="checkbox" data-value="4" data-group="user" '.('r' == substr($info, 1, 1) ? ' checked' : '').'></td>
                            <td><input id="u_2" type="checkbox" data-value="2" data-group="user" '.('w' == substr($info, 2, 1) ? ' checked' : '').'></td>
                            <td><input id="u_1" type="checkbox" data-value="1" data-group="user" '.('x' == substr($info, 3, 1) ? ' checked' : '').'></td>
                        </tr>
                        <tr>
                            <td>'.trans('Group').'</td>
                            <td><input id="g_4" type="checkbox" data-value="4" data-group="group" '.('r' == substr($info, 4, 1) ? ' checked' : '').'></td>
                            <td><input id="g_2" type="checkbox" data-value="2" data-group="group" '.('w' == substr($info, 5, 1) ? ' checked' : '').'></td>
                            <td><input id="g_1" type="checkbox" data-value="1" data-group="group" '.('x' == substr($info, 6, 1) ? ' checked' : '').'></td>
                        </tr>
                        <tr>
                            <td>'.trans('All').'</td>
                            <td><input id="a_4" type="checkbox" data-value="4" data-group="all" '.('r' == substr($info, 7, 1) ? ' checked' : '').'></td>
                            <td><input id="a_2" type="checkbox" data-value="2" data-group="all" '.('w' == substr($info, 8, 1) ? ' checked' : '').'></td>
                            <td><input id="a_1" type="checkbox" data-value="1" data-group="all" '.('x' == substr($info, 9, 1) ? ' checked' : '').'></td>
                        </tr>
                        <tr>
                            <td></td>
        ...
        case 'change_lang':
            ...
            if (array_key_exists($choosen_lang, $languages)) {
                if (!file_exists($langDir.$choosen_lang.'.php')) {
                    response(trans('Lang_Not_Found').AddErrorLocation())->send();
                    exit;
                } else {
        ...
        case 'get_file': // preview or edit
            ...
            if ('preview' != $sub_action && 'edit' != $sub_action) {
                response(trans('wrong action').AddErrorLocation())->send();
                exit;
            }

            if ($ftp) {
                $selected_file = ('preview' == $sub_action ? $config['ftp_base_url'].$config['upload_dir'].$_GET['file'] : $config['ftp_base_url'].$config['upload_dir'].$_POST['path']);
            } else {
                $selected_file = ('preview' == $sub_action ? $config['current_path'].$_GET['file'] : $config['current_path'].$_POST['path']);
            ...
            if ('text' == $preview_mode) {
                $is_allowed = ('preview' == $sub_action ? $config['preview_text_files'] : $config['edit_text_files']);
                $allowed_file_exts = ('preview' == $sub_action ? $config['previewable_text_file_exts'] : $config['editable_text_file_exts']);
            } elseif ('google' == $preview_mode) {
            ...
            if (!in_array($info['extension'], $allowed_file_exts)
                ...
                response(sprintf(trans('File_Open_Edit_Not_Allowed'), ('preview' == $sub_action ? strtolower(trans('Open')) : strtolower(trans('Edit')))).AddErrorLocation())->send();
                exit;
            }
            if ('preview' == $sub_action) {
                if ('text' == $preview_mode) {
                    // get and sanities
